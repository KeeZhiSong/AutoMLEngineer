...
def build_html(data: dict) -> str:
    ...
    h = ['<title>The Search Ledger</title>',
         ...
         '<div class="stat"><span class="v">0.6038</span>'
         '<span class="l">Best unaided, 20-seed</span></div>',
    ...
    h += ['</svg></figure>',
          '<div class="callout"><p>The best of these measures <b class="mono">0.6038</b> '
          'over 20 seeds (sd 0.0003), which <b>matches the human-tuned result exactly</b> '
          '&mdash; by a different route. The human path was a listwise objective trained on '
          'evaluation-length lists plus a retuned learning rate; the agent reached the same '
          'score through curriculum learning over sequence length and a length-normalisation '
          'feature. It did not rediscover the human path.</p></div></section>']
    ...
    h += ['<section class="reveal"><div class="eyebrow">Why the accept margin exists</div>',
          ...
          'same numbers and spent an hour chasing it.</p>'
          '<p style="margin-top:18px;color:var(--muted)">The same rule applied in the other '
          'direction on the final run. An accepted result read <b class="mono">0.6040</b> on '
          'one seed, and a neighbouring configuration in the same sweep read 0.6040 and '
          'measured 0.6034 across five. So it was tested at <b>20 seeds before anything was '
          'claimed</b>: mean 0.6038, sd 0.0003, every seed above the baseline. The caution '
          'was right to apply, and this time the result survived it.</p></div></section>']
    ...
    h += ['</tbody></table></div></section>',
          ...
          'Baseline 0.6016 &middot; agent 0.6038 (20-seed) &middot; human-tuned 0.6038 &middot; '
