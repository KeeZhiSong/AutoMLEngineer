...
**Live search ledger:** <https://keezhisong.github.io/AutoMLEngineer/> — all 387
experiments, the submitted result, and every run, generated from the logs by CI.
...
### → **<https://keezhisong.github.io/AutoMLEngineer/>**

Every experiment this project ever ran, on one page. A grid of one cell per
experiment coloured by outcome, the submitted result, where each accepted result
landed, the score timeline, the architecture generations including the two that
failed, token spend by role, and the full run table.

It is **generated, never hand-written**, from the run logs themselves:
...
A GitHub Actions workflow (`.github/workflows/ledger.yml`) regenerates it on
every push that touches a run log, so the published page cannot drift from the
data it describes. That is the same rule the rest of the project follows: the
artefact is derived from the measurement, never asserted alongside it. Served
from `main` + `/docs`.
...
| published search ledger | **<https://keezhisong.github.io/AutoMLEngineer/>** (source: [`docs/index.html`](docs/index.html), generated by `tools_build_ledger.py`) |
