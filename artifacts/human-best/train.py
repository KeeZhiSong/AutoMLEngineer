...
def listwise_softmax(model: FM, X: np.ndarray, y: np.ndarray,
                     user_id: np.ndarray) -> tuple[np.ndarray, float]:
    """Normalised-target listwise softmax over each group.

    The metrics rank WITHIN a user, so the objective should too. Targets are
    normalised, t = y_g / y_g.sum(), which makes each group contribute equally
    regardless of how many positives it holds; the raw (unnormalised) form was
    measured to be actively harmful.

    Groups with no positive label carry no ranking information -- 27.1% of
    evaluation users are all-negative -- so they get zero gradient rather than a
    degenerate uniform target.

    Gradient contract: returns dLoss/dlogit per ROW of X, in the caller's order.
    """
    z, _, _ = model.logits(X)
    order, starts, lengths = group_segments(user_id)

    zs = z[order]
    ys = y[order].astype(np.float64)
    p = segment_softmax(zs, starts, lengths)

    tot = np.repeat(segment_sum(ys, starts), lengths)
    scored = tot > 0
    t = np.zeros_like(ys)
    np.divide(ys, tot, out=t, where=scored)

    n_groups = max(int(np.count_nonzero(segment_sum(ys, starts) > 0)), 1)
    dzs = np.where(scored, (p - t) / n_groups, 0.0)

    dz = np.empty_like(dzs)
    dz[order] = dzs                                  # back to the caller's order
    loss = float(-np.sum(t[scored] * np.log(p[scored] + 1e-9)) / n_groups)
    return dz.astype(np.float32), loss
...
def fit(model: FM,
    ...
    batch_mode = str(cfg.get("batch_mode", "user"))
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        if batch_mode == "user":
            batches = user_batches(train_enc.user_id, bs, rng,
                                   group_size=cfg.get("group_size", 5))
        else:
